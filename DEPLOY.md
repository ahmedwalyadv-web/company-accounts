...
4. في الإعدادات:
   ...
   - **Start Command:** `npm run db:init && npm start` (الأمر ده بيحدّث هيكل قاعدة البيانات تلقائيًا مع كل نشر جديد، آمن يتكرر كل مرة)
   - **Instance Type:** Free (أو Starter لو عايز السيرفر يفضل سريع ومايناموش)
...
## 6) تفعيل المساعد الافتراضي (اختياري)

النظام فيه مساعد افتراضي (نص وصوت) بيقدر يجاوبك عن أرصدة الخزنة والمخزون والتقارير، وينفذلك عمليات (مصروفات/استلام فلوس/فواتير شراء وبيع) بعد تأكيدك عليها. عشان يشتغل:

1. اعمل حساب على [console.anthropic.com](https://console.anthropic.com) واطلع مفتاح API (API Key).
2. من داخل النظام، روح لصفحة **إعدادات الشركة** وحط المفتاح في خانة "مفتاح Anthropic API" واحفظ.
3. روح لصفحة **المساعد الافتراضي** من القائمة وابدأ تتكلم أو تكتب.

المتصفح المفروض يدعم الميكروفون (Chrome/Edge بيدعموا ده بشكل أساسي). لو مفيش مفتاح محفوظ، صفحة المساعد هتوضح لك ده وتوجهك لصفحة الإعدادات.
